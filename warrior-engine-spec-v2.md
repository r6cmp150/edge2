...
**Known limitation, corrected 2026-09-05 — this paragraph was itself stale.** It previously claimed `stalenessDays` was "now also a HARD BOUND... beyond 180 days... reads not-checked," describing the bound from its brief 2026-09-04 life before that same day's own re-measurement dropped it (see "180-day staleness bound — added, then dropped" below) — this paragraph was never updated after the drop, and direct code inspection 2026-09-05 confirmed no bound of any kind existed in `evaluatePillarFloat` (`engines/warrior/gate.js`) or `core/float-table.js` at that point. The actual state, current as of this correction:

EntityPublicFloat is annual-only (10-K cover page) — its reference date is typically 6-18 months old relative to a live check, on top of however old the table itself is. `stalenessDays` (vs. the filing's own reference date) and the table's own age (vs. its `builtAt`) are two separate staleness numbers, both surfaced. The volume-consistency backstop (`VOLUME_TO_FLOAT_MULTIPLE_MAX`, `engines/warrior/gate.js`) is the PRIMARY defense against a stale-but-small implied float passing the gate — but it depends on `todayVolume`, which is only populated when RVOL itself is checkable (session `OPEN`, well past the open). Found live 2026-09-05, in the Phase 7 signal logger's first real dry run: a CLOSED-session scan supplied no `todayVolume`, the volume check never ran at all, and a 432-day-old implied float (GPRO) was gated on a raw threshold comparison with zero staleness defense — the exact BIAF/GRI shape this whole investigation exists to prevent, caught by luck (GPRO failed anyway) rather than by construction. Re-added a 180-day bound (`STALENESS_FALLBACK_BOUND_DAYS`, `engines/warrior/gate.js`) — but scoped as a FALLBACK, firing only when `todayVolume` is unavailable, not a blanket bound. When volume data IS available, the tighter, age-blind volume-consistency check remains the sole gate, preserving the coverage the original drop decision was protecting; the fallback only closes the specific gap that decision's data didn't examine (PRE/CLOSED/AH-session scans, which a scheduled server-side logger runs routinely, unlike the interactive client that decision was reasoned against).
